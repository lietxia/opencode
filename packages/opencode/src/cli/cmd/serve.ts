...
export const ServeCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.serve")(function* (args) {
    ...
    const discovery = args.discoverable
      ? yield* ServerDiscovery.Service.pipe(Effect.provide(ServerDiscovery.defaultLayer), Effect.orDie)
      : undefined
    if (discovery) {
    ...
    yield* Effect.never.pipe(
      Effect.ensuring(
        discovery
          ? discovery
              .remove()
              .pipe(Effect.orDie, Effect.ensuring(Effect.sync(() => process.off("exit", ServerDiscovery.removeSync))))
          : Effect.void,
      ),
    )
  }),
