...
// read the secret
const ret = await $`npx tsx -e "import('@opencode-ai/script').then(m => m.sst?.secretList?.({stage:'frank'}))" 2>/dev/null || bun sst secret list --stage frank`.cwd(root).text()
...
await $`npx tsx -e "import('@opencode-ai/script').then(m => m.sst?.secretLoad?.({stage:'${stage}'}))" 2>/dev/null || bun sst secret load ${envFilePath} --stage ${stage}`.cwd(root)
